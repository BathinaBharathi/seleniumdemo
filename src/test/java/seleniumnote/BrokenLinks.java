...
public class BrokenLinks {
	...
	public static void main(String[] args) throws IOException {
		...
		List<WebElement> links=driver.findElements(By.tagName("a"));  // capture the all links
		System.out.println("Total number of links:"+links.size()); //total no of links 48
		
		int brokenlinks=0;
		
		for(WebElement linkEle:links)
		{
			String hrefAttValue=linkEle.getAttribute("href");  //grtting href attribute
			...
			HttpURLConnection conn=(HttpURLConnection) linkurl.openConnection(); //open to the connection
			...
			if(conn.getResponseCode()>=400)
			{
				...
				brokenlinks++;  //whenever broken value incresas
			}
			else
			{
